...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5514991357606:+55 (55) 991357606\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '😐 aguarde...se der erro tente novamente 😐',
				success: '✔️ Sucesso ✔️',
                                levelon: '❬ ✔ ❭ *habilitar 1 ponto  Level*',
				leveloff: ' ❬ X ❭  *desabilitar 1 ponto Level*',
				levelnoton: '❬ X ❭ *level ainda nao  ativo*',
				levelnol: '*Pqp kskst level* 0 ',
				error: {
					stick: '[❗] Falha, ocorreu um erro ao converter a imagem em um STIKER tente novamente 🥺',
					Iv: '⚠️ Link inválido ⚠️'
				},
				only: {
					group: '[❗] presisa ta em grupo burro🤦‍♂️ ⚠️',
					ownerG: '[❗] Este comando só pode ser usado pelo grupo proprietário! ⚠️',
					ownerB: '[❗] Este comando só pode ser usado pelo Toin! ⚠️',
					admin: '[❗] Este comando só pode ser usado por administradores de grupo! ⚠️',
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador! ⚠️',
                                        rgB: `── 「REGISTRE-SE」 ──\nOlá como!\nVocê não está registrado no banco de dados, \n\nComando : ${prefix}rg nome|idade\nExemplo : ${prefix}rg william|11`,
				}
			}
			...
			const ownerNumber = ["5514991357606@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5514991357606@s.whatsapp.net'
                        /******ApiKey Input******/
	    ...
	    case 'kapankah':
			...
			case 'closegc':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`⚠️ Falhou, no momento da conversão ${tipe} para o sticker`)
							})
							.on('end', function () {
								console.log('Finish')
				...
				case 'toimg':
				    client.updatePresence(from, Presence.composing)
                                    if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedSticker) return reply('⚠️ Apenas Stickers ⚠️')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('⚠️ Falha ao converter Stickers em imagens tente novamente ⚠️')
                	...
                	client.updatePresence(from, Presence.composing) 
                        if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedVideo) return reply('⚠️ responder vídeo hum ⚠️')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
               ...
               if (anu.error) return reply(anu.error)
				...
				if (isUser) return reply(mess.only.daftarB)
					...
					break
                                case 'nsfwneko':
				    try{
						if (!isNsfw) return reply('⚠️ *NSFW NAUM ATIVADO* ⚠️')
                                                if (!isUser) return reply(mess.only.daftarB)
            ...
            break
                                ...
                                case 'nsfwtrap':
                                        try{
                                                if (!isNsfw) return reply('⚠️ *NSFW Desativado* ⚠️')
                                                if (!isUser) return reply(mess.only.daftarB)
