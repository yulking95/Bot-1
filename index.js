...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const premium = ["558981210976@s.whatsapp.net","558981246187@s.whatsapp.net","558981204807@s.whatsapp.net","558981213553@s.whatsapp.net"]
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("Alguem ai pra jogar?")){
			...
			switch(command) {
	            case 'menu':
                    putagg = await getBuffer(`https://i.ibb.co/zXZpwLd/Bot-Anna.jpg`)
...
// recursos anime, random @✞︎𝙼𝚎𝚕𝚒𝚘𝚍𝚊𝚜✞︎ domina vida
          ...
          reply('Espere..')
				...
				case 'bot':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*_Comandos para instalar o meu bot \npkg install git\npkg install ffmpeg\npkg install wget\npkg install nodejs\ngit clone https://github.com/Meliodas-rai/Bot\ncd Bot\nbash install.sh\nnpm start\escaneie o qr code q ira aparecer\naperte o botão "Ctrl" depois aperte "C" no teclado\ndigite\npkg install pm2\nnpm i -g pm2\npm2 start index.js\npm2 monit\n_seja feliz meu parsa😎\n\nOBS:vc vai precisar do termux e de 2 celulares'})
					break
				case 'belle3':
					memein = await kagApi.memeindo()
        ...
        case 'promover':
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ERROR]','red'), 'eita pexti vey omando n registrado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
