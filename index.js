...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Meliodas\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=558981210976:+55 89 81210976\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '*'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝙊𝙡𝙖́ 𝙢𝙚𝙪 𝙘𝙖𝙧𝙤 𝙜𝙖𝙛𝙖𝙣𝙝𝙤𝙩𝙤 @${num.split('@')[0]}\n𝙎𝙚𝙟𝙖 𝙗𝙚𝙢 𝙫𝙞𝙣𝙙𝙤 𝙖𝙤 𝙜𝙧𝙪𝙥𝙤  *${mdata.subject}*\n\n𝙀𝙨𝙥𝙚𝙧𝙤 𝙦𝙪𝙚 𝙜𝙤𝙨𝙩𝙚 ❤️`
				...
				teks = `𝙁𝙡𝙬, 𝙫𝙖𝙞 𝙥𝙚𝙡𝙖 𝙨𝙤𝙢𝙗𝙧𝙖 𝙥𝙦 𝙗𝙤𝙨𝙩𝙖 𝙣𝙤 𝙨𝙤𝙡 𝙨𝙚𝙘𝙖@${num.split('@')[0]} 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5589210976@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5589210976@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5589210976@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5589210976@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5589210976@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("Averaaulahojeas")){
			client.updatePresence(from, Presence.composing)
			reply("Aham")
	}
		...
		if (messagesC.includes("Borajogar")){
			client.updatePresence(from, Presence.composing)
			reply("Não, eu tô sem tempo")
	}
